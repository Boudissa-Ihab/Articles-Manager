...
class EditDocument extends Component
{
    ...
    {
        return [
            'title' => ['required', 'string', 'min:3', Rule::unique('App\Models\Document', 'title')->ignore($this->document->id)],
            ...
            'featured' => 'boolean',
            'path' => 'nullable|file|mimes:pdf|max:204800',
    ...
    public function mount()
    {
        ...
        foreach($this->document->domains as $domain)
            $this->selectDomains[$domain->id] = $domain->id;
        foreach($this->document->authors as $author)
            $this->selectAuthors[$author->id] = $author->id;
        $this->temporaryDomains = $this->selectDomains;
        $this->temporaryAuthors = $this->selectAuthors;
    }
    ...
    {
        $domains = Domain::get();
        foreach($this->selectDomains as $domain)
            $domains = $domains->except($domain);
        return $domains;
    }
    ...
    {
        $authors = Author::get();
        foreach($this->selectAuthors as $author)
            $authors = $authors->except($author);
        return $authors;
    }
    ...
    public function updatedAuthor($value)
    {
        ...
        {
            $this->selectAuthors[$value] = intval($value);
        ...
        if($this->validate())
        {
            try{
                DB::transaction(function () {
                    ...
                    // Before assigning new domains & authors to document
                    // Need to detach old domains & authors from document
                    $document->domains()->detach($this->temporaryDomains);
                    $document->authors()->detach($this->temporaryAuthors);
                    ...
                    alert()->success('Document modifié avec succès');
                    return redirect()->route('admin.documents');
                });
            } catch(Throwable $th) {
                alert()->error("Impossible de modifier ce document");
            }
        }
    }
    ...
    {
        return view('livewire.admin.pages.edit-document');
    }
}
